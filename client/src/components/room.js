...
const Room = ({ location }) => {
    ...
    const sendCashPayment = (event) => {
        ...
        if (cashSelect) {
            socket.emit('cash_payment', cashRequest.from.playerID, cashSelect, () => {
    ...
    const playCard = (index) => {
        if (turnStatus && gameStatus === 'ongoing') {
            ...
            if(hand[index].action === 4){
                setActionPrompt('sly_deal')
                setPlayedCardIndex(index)
            }

            else{
                socket.emit('place_card', index)
    ...
    const sendOpponentActionResponse = (e, actionResponse) => {
        ...
        if(opponentAction){
            socket.emit('opponent_action_response', opponentAction, actionResponse, () => {
                setOpponentAction(null)
            })
        }

        else if(cashRequest){
            socket.emit('opponent_action_response', cashRequest, actionResponse, () => {
                setCashRequest(null)
            })
        }
    ...
    if (messageHistory.length > 0) {
        var chatDisplay = <ChatDisplay messages={messageHistory} user_name={name} />
    }
    ...
    if (hand.length > 0) {
        var handList = <HandViewer hand={hand} turnStatus={turnStatus} playCard={playCard} />
    }

    if (gameStatus === 'ongoing') {
        var turnMessage = <div>{(turnStatus) ? `You have ${chances} chances left.` : "It is not your turn."}</div>
    }

    //modal for sly deal card selection
    if(actionPrompt === 'sly_deal'){
        var actionModal = 
        <SlyDealModal opponentPlayed={opponentPlayed} setActionPrompt={setActionPrompt} setCardSelect={setCardSelect} />
    }

    //modal for opponentAction (i.e playing Nein card or allowing action)
    if(opponentAction){
        var actionModal = <OpponentActionModal opponentAction={opponentAction} sendOpponentActionResponse={sendOpponentActionResponse} hand={hand} />
    }
    ...
    if( gameStatus === 'waiting'){
        return(
                <WaitingRoom chatDisplay={chatDisplay} chatMessage={chatMessage} setChatMessage={setChatMessage} sendChatMessage={sendChatMessage} leaveGame={leaveGame} sendReadyStatus={sendReadyStatus} gameStatus={gameStatus} playerList={playerList} room={room} name={name} ready={ready}/>
        )
    }
        ...
        <div className="game-screen-wrapper">
            <div className="game-area-wrapper">
                <div className="opponent-view game-area-container">
                {personal_played}
                    {opp_played}
                </div>
                <div className="game-overview-container game-area-container">
                    ...
                    <div>
                    {
                        (gameStatus === 'ongoing' && turnStatus) ? <button className='end-turn-button' onClick={e => endTurn(e)} title="You can end your turn early">End turn</button> : null
                    }
                    </div>
                    ...
                    <div className='timer-wrapper'>
                        {(turnStatus) ? <><i className="fas fa-stopwatch" style={{color: "var(--accent)", marginRight: 8}}></i> 
                    {timerSeconds}</> : null}
                    
                    </div>
                
                </div>
                <div className="personal-view game-area-container">
